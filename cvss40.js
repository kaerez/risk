...
class Vector {
    ...
    */
    ...
    get raw() {
        ...
        // Add standard CVSS metrics first, in their correct order, if they exist in the instance
        ...
        // Add any other metrics (from extensions) that were not part of the standard set
        for (const key in this.metrics) {
            if (!processedMetrics.has(key)) {
                const value = this.metrics[key];
                // Extensions might not use 'X' for 'Not Defined', so we just check for presence
                if (value !== undefined) {
                     metricEntries.push(`/${key}:${value}`);
                }
            }
        ...
        return baseString + metricEntries.join('');
    }
    ...
    validateStringVector(vector) {
        ...
        if (parts.length === 0) {
            // It's a valid base string, but has no metrics. Let updateMetricsFromVectorString handle it.
            return true;
        }

        const providedMetrics = new Map();
        for(const part of parts) {
            const [key, value] = part.split(':');
            if (!key || value === undefined) {
                console.error(`Error: invalid metric format in vector: ${part}`);
                return false;
            }
            if (providedMetrics.has(key)) {
                console.error(`Error: duplicate metric in vector: ${key}`);
                return false;
            }
            providedMetrics.set(key, value);
        }

        // Check for mandatory Base metrics
        const mandatoryMetrics = Object.keys(Vector.METRICS.BASE);
        for (const metric of mandatoryMetrics) {
            if (!providedMetrics.has(metric)) {
                console.error(`Error: invalid vector, missing mandatory metric: ${metric}`);
                return false;
            }
        }

        // Check if the values for all known CVSS metrics are valid
        for (const [key, value] of providedMetrics.entries()) {
            if (Vector.ALL_METRICS[key]) { // If it's a known CVSS metric
                if (!Vector.ALL_METRICS[key].includes(value)) {
                    console.error(`Error: invalid value for metric ${key}: ${value}`);
                    return false;
                }
            }
            // For unknown (extension) metrics, we don't validate the value, just that it exists.
            // This allows them to be processed by the extension logic in app.js.
        }
    ...
    updateMetricsFromVectorString(vector) {
        ...
        // FIXED: Reset metrics to default before applying new ones.
        this.resetMetrics();
    ...
    updateMetric(metric, value) {
        // Allow adding new (extension) metrics, not just updating existing ones.
